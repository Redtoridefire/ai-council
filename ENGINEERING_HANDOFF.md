...
- Evidence injection (RAG) pipeline via local `docs/` ingestion (`.md`, `.txt`, `.json`, `.pdf`), chunking, lightweight token-based cosine retrieval (no heavy ML runtime dependency).
...
- Deployment is optimized for Railway by keeping `requirements.txt` minimal for runtime (Telegram + council engine).
- Optional dashboard dependencies were split into `requirements-dashboard.txt` to avoid bloating production image size.
- `railway.json` configures Railway to install core dependencies and start `telegram_bot.py` directly.
